...
@interface CBBlackViewController ()

@property (strong, nonatomic) NSMutableArray *list;

@end

@implementation CBBlackViewController
{
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        [self.tabBarItem setTitle:@"Blacklist"];
    }
    return self;
}
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        // 先删除数据库中的数据
        NSString *sqlStr = @"delete from blacklist where number=?";
...
- (void)updateDataSource
{
    ...
    [queue inDatabase:^(FMDatabase *db){
        // 查询数据
        NSString *sqlStr = @"select name, number from blacklist";
        ...
        if ([blacklist count] > 0)
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [NSKeyedArchiver archiveRootObject:self.list toFile:pathInDocumentDirectory(kBlacklistArchive)];
            });
